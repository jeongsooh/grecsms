...
def calculate_price(start_time, end_time, energy):
  ...
  # print("전체 {}시간 동안 평균 {}kW로 충전: 충전매출 {}원".format(total_hours, average_energy, energy * charging_rate))
  ...
  base_fee = price_table[HV][BASIC] * cat_charger / (30 * 24 * util_rate) * total_hours
  ...
  while current_datetime < end_datetime:
    ...
    if current_datetime == start_datetime:
      if end_datetime - current_datetime < timedelta(seconds=3600):
        start_end_flag = 3
        actual_min = end_datetime - current_datetime
      else:
        start_end_flag = 1
    ...
    price = calculate_price_for_date_and_time(date_obj, time_obj, start_end_flag, actual_min, average_energy)
    total_price += price
...
def calculate_price_for_date_and_time(date_obj, time_obj, start_end_flag, actual_min, average_energy):
  ...
  elif start_end_flag == 3:
    actual_rate = actual_min.total_seconds() / 60 / 60
  else:
    actual_rate = 1
